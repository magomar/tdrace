...
fn test_all_categories_surface_dynamics_degradation() {
    ...
    for (id, config) in test_vehicles {
        // 1. Acceleration: Asphalt must be quicker or reach higher speed than DeepSand
        let a_asphalt = run_protocol_a(&config, SurfaceType::Asphalt, 6.0, DEFAULT_SIMULATION_DT);
        let a_sand = run_protocol_a(&config, SurfaceType::DeepSand, 6.0, DEFAULT_SIMULATION_DT);
        assert!(
            a_asphalt.v_terminal_kmh > a_sand.v_terminal_kmh,
            "{}: Asphalt terminal speed ({:.1} km/h) should exceed DeepSand ({:.1} km/h)",
            id,
            a_asphalt.v_terminal_kmh,
            a_sand.v_terminal_kmh
        );

        // 2. Braking: Stopping distance on SheetIce must be > 2x Asphalt, and Concrete must be close to Asphalt
        ...
        let b_ice = run_protocol_b(&config, SurfaceType::SheetIce, 100.0, DEFAULT_SIMULATION_DT);
        assert!(
            b_ice.stopping_distance_m > b_asphalt.stopping_distance_m * 2.0,
            "{}: Stopping distance on SheetIce ({:.1}m) should exceed 2x Asphalt ({:.1}m)",
            id,
        ...
        // 3. Coast-down: Asphalt must roll further than DeepSand due to rolling resistance
        let e_asphalt = run_protocol_e(&config, SurfaceType::Asphalt, 120.0, DEFAULT_SIMULATION_DT);
        let e_sand = run_protocol_e(&config, SurfaceType::DeepSand, 120.0, DEFAULT_SIMULATION_DT);
        assert!(
            e_asphalt.coast_distance_m > e_sand.coast_distance_m,
            "{}: Coast distance on Asphalt ({:.1}m) should exceed DeepSand ({:.1}m)",
            id,
            e_asphalt.coast_distance_m,
            e_sand.coast_distance_m
        );

        // 4. Skidpad: Asphalt cornering limit must realistically exceed SheetIce (> 5x) and be >= 0.85g
        let c_asphalt = run_protocol_c(&config, SurfaceType::Asphalt, 30.0, DEFAULT_SIMULATION_DT);
        let c_ice = run_protocol_c(&config, SurfaceType::SheetIce, 30.0, DEFAULT_SIMULATION_DT);
        assert!(
            c_asphalt.peak_lateral_accel_g > c_ice.peak_lateral_accel_g * 5.0,
            "{}: Asphalt lateral g ({:.2}g) should exceed 5x SheetIce ({:.2}g)",
            id,
