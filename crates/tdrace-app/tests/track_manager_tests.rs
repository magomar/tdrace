...
fn test_track_categories_initial_presets() {
    ...
    // 1. Initial state: 54 main presets across modules (10 Classic + 14 unique F1 + 10 unique Rally + 8 famous Kart + 12 Nascar), 0 drafts
    let main_tracks = manager.main_track_choices();
    let draft_tracks = manager.draft_track_choices();

    assert_eq!(main_tracks.len(), 54, "All 54 presets across modules should be Main tracks");
...
fn test_draft_creation_and_isolation_from_main_menu() {
    ...
    let main_tracks = manager.main_track_choices();
    assert_eq!(main_tracks.len(), 54, "Main menu should only contain approved circuits");
    let draft_tracks = manager.draft_track_choices();
...
fn test_promotion_and_demotion_lifecycle() {
    ...
    assert_eq!(manager.main_track_choices().len(), 55, "Promoted track must appear in Main");
    ...
    assert_eq!(manager.main_track_choices().len(), 54, "Demoted track must be removed from Main");
...
fn test_module_filter_filtering_and_presets_in_classic() {
    ...
    // Initial state: 54 tracks across all modules (10 Classic + 14 unique F1 + 10 unique Rally + 8 famous Kart + 12 Nascar)
    assert_eq!(manager.main_track_choices().len(), 54);
    ...
    assert_eq!(manager.main_track_choices().len(), 56);
    assert_eq!(manager.filtered_main_track_choices(ModuleFilter::Classic).len(), 10);
...
fn test_module_subdirectories_and_file_movement() {
    ...
    assert_eq!(new_scanner.main_track_choices().len(), 55); // 54 presets + 1 custom
...
fn test_track_manager_clone_preset_to_drafts() {
    ...
    // Initial check: 54 main tracks, 0 drafts
    assert_eq!(manager.main_track_choices().len(), 54);
    assert_eq!(manager.draft_track_choices().len(), 0);
    ...
    // Verify drafts list has 1 track, main still has 54
    assert_eq!(manager.main_track_choices().len(), 54);
