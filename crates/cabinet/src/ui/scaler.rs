...
impl UiScaler {
    ...
    /// Minimum ergonomic touch target size in pixels (44 CSS pixels / dp standard).
    ...
    /// Computes centered rectangle coordinates.
    #[inline]
    pub fn centered_rect(&self, w: f32, h: f32) -> (f32, f32, f32, f32) {
    ...
    /// Draws an interactive button container with distinct focus, hover, and active feedback.
    pub fn draw_button_card(
        &self,
        x: f32,
        y: f32,
        w: f32,
        ...
        let bg_col = if is_active {
            Color::new(accent_col.r * 0.25, accent_col.g * 0.25, accent_col.b * 0.25, 0.95)
        } else {
            Color::new(0.08, 0.10, 0.15, 0.90)
        };
        let border_col = if is_active {
            accent_col
        } else {
            Color::new(0.25, 0.35, 0.45, 0.80)
        };
        let thickness = if is_active { 2.4 } else { 1.2 };
        self.draw_glass_card(x, y, w, h, bg_col, border_col, thickness);
    }
}
